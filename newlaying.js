...
function createHeaderCanvas(rows) {
  ...
  hctx.fillText(`Survey for New OFC Laying: ${routeName}`, hc.width / 2, 30);
  ...
  hctx.fillText(`Total laying length - ${totalNewLength/1000} KM`, hc.width / 2, 55);
  ...
  hctx.setLineDash([]); hctx.fillText('Planned Laying Portion', legendX + legendLineLength + 10, legendYStart + legendGap + 5);
  ...
  hctx.lineWidth = 2; hctx.fillText('Planned Laying Length(HDD)', legendX + legendLineLength + 10, legendYStart + 2 * legendGap + 5);
  ...
  hctx.setLineDash([]); hctx.lineWidth = 2; hctx.fillText('Planned Laying Length(Trenching)', legendX + legendLineLength + 10, legendYStart + 3 * legendGap + 5);
...
function drawLowerOFCOnCtx(rctx, row, rowY, lowerOFCGap, roadGap) {
  ...
  rctx.font = 'bold 11px Arial';
  rctx.textAlign = 'center';
  if (row && row.length) rctx.fillText('LAYING', Math.max(8, row[0].startX - 30), lowerY);
  rctx.textAlign = 'left';

  for (const seg of row) {
    if (seg.condition === 'new') {
      rctx.lineWidth = 4;
      rctx.strokeStyle = 'black';
...
function drawTechniqueMarkingsOnCtx(rctx, row, rowY, roadGap, techniqueGap) {
  ...
  for (let i = 0; i < row.length; i++) {
    const segment = row[i];
    if (segment.condition === 'new' && segment.technique) {

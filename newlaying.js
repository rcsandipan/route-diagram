...
function drawDiagram() {
  ...
  for (const row of rows) {
    for (const segment of row) {
      if (segment.condition === 'new') totalBadLength += segment.length;
    }
  }
  ...
  ctx.fillText(`Survey for OFC Laying: ${routeName}` || 'Unnamed Route', canvas.width / 2, 40);

  ctx.textAlign = "left";
  ctx.fillStyle = "black";
  ctx.font = "16px Arial";
  ctx.textAlign = "center";
  ctx.fillText(`Total length - ${totalBadLength} Metre`, canvas.width / 2, 65);
  ctx.textAlign = "left";
...
//   ctx.fillText("Existing OFC Route", legendX + legendLineLength + 10, legendYStart + 5);
  ...
  ctx.stroke();
  ctx.setLineDash([]);
  ctx.fillText("Planned Laying Portion", legendX + legendLineLength + 10, legendYStart + legendGap + 5);
  ...
  ctx.fillText("Planned Laying Length(HDD)", legendX + legendLineLength + 10, legendYStart + 2 * legendGap + 5);
  ...
  ctx.setLineDash([]);
  ctx.lineWidth = 2; // reset line width
  ctx.fillText("Planned Laying Length(Trenching)", legendX + legendLineLength + 10, legendYStart + 3 * legendGap + 5);
...
function drawLowerOFCForRow(row, rowY, lowerOFCGap, roadGap) {
  ...
  ctx.font = "bold 11px Arial";
  ctx.textAlign = "center";
  ctx.fillText("LAYING", row[0].startX - 30, lowerY);
  ctx.textAlign = "left";
